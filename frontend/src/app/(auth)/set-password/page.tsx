...
function SetPasswordForm() {
    ...
    return (
      <div className="login-form-card">
        <div className="login-form-header">
          <h2 className="login-form-title">Password Setup Required</h2>
          <p className="login-form-subtitle">
            Your account requires a password to be set. Please use the invite link sent to your email.
          </p>
        </div>
        <button className="login-btn" onClick={() => router.push("/login")} style={{ marginTop: "1rem" }}>
          Back to Sign In
        </button>
      </div>
    );
  ...
  return (
    <div className="login-form-card">
      <div className="login-form-header">
        <h2 className="login-form-title">Set Your Password</h2>
        <p className="login-form-subtitle">Choose a secure password for your Nyayo Sentinel account.</p>
      </div>

      {success ? (
        <div style={{ background: "#dcfce7", color: "#15803d", padding: "0.75rem 1rem", borderRadius: "0.4rem", marginBottom: "1rem" }}>
          ...
          <form onSubmit={(e) => void handleSubmit(e)} className="login-form">
            <div className="login-field">
              ...
              <div className="login-input-wrap">
                ...
                />
                <button type="button" className="login-eye" onClick={() => setShowPassword((v) => !v)} aria-label="Toggle password">
                  {showPassword ? (
            ...
            <button type="submit" className="login-btn" disabled={loading}>
              {loading ? <span className="login-spinner" /> : "Set Password"}
            </button>
          </form>
        </>
      )}
